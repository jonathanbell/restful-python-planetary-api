...
@app.route("/planet/update", methods=["PUT"])
@jwt_required
def update_planet():
...
@app.route("/planet/remove/<int:planet_id>", methods=["DELETE"])
@jwt_required
def planet_remove(planet_id: int):
    planet = Planet.query.filter_by(planet_id=planet_id).first()
    if planet:
        # remove the planet
    ...
    return jsonify(message="Cannot find that planet!"), 404
